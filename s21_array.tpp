...
array<T, amount>::array(std::initializer_list<value_type> const &items) {
  size_type i = 0;
  for (const_iterator it = items.begin(); it != items.end() && i < amount;
       ++it, ++i) {
    usual_array[i] = *it;
  }
  for (size_type j = i; j < amount; ++j) {
    usual_array[j] = 0;
  }
}
...
array<T, amount>::array(const array &a) {
  for (size_type i = 0; i < amount; i++) usual_array[i] = a.usual_array[i];
}
